...
export const formatSpeed = (value, unit, t) => {
	switch (unit) {
		case 'kmh':
			return `${(value * 1.852).toFixed(2)} kmh`;
		case 'mph':
			return `${(value * 1.15078).toFixed(2)} mph`;
		case 'kn':
		default:
			return `${(value * 1).toFixed(2)} kn`;
	}
};
...
export const formatVolume = (value, unit) => {
	switch (unit) {
		case 'impGal':
			return `${(value / 4.546).toFixed(2)} impGal`;
		case 'usGal':
			return `${(value / 3.785).toFixed(2)} usGal`;
		case 'ltr':
		default:
			return `${(value / 1).toFixed(2)} ltr`;
	}
};
